...
if __name__ == '__main__':
    ...
    # apikey = "7f05eee4-f6de-4b6d-800f-cea4da2f36ce"
    # secretkey = "80889D4C92CECA8D4BB22123D36F27C5"
    # IP = "0"
    # 备注名 = "read"
    # 权限 = "只读"
    api_key = "7f05eee4-f6de-4b6d-800f-cea4da2f36ce"
    secret_key = "80889D4C92CECA8D4BB22123D36F27C5"
    passphrase = "NLXQBnlxqb2727"
...
# 资金账户API
    ...
    # 单一币种账户信息
    result = accountAPI.get_currency('WGRT')
    # 资金划转
...
# 获取系统升级状态
    ...
    for curreny in result :
        if curreny['currency'] == 'WGRT':
            print("{} in OKex can_deposit:{},can_withdraw:{}".format(curreny['currency'],curreny['can_deposit'],curreny['can_withdraw']))

            itchat.auto_login(hotReload = False) #一直登陆

            # name = itchat.search_friends(name=u'石头')# 返回的是关于备注为小明的好友的所有信息。返回的是一个列表，如下所示。
            # print("name=:",name)
            # bao = name[0]["UserName"]  #也就获取那一串的数字加字母
            name = itchat.search_friends(name=u'大号')# 返回的是关于备注为小明的好友的所有信息。返回的是一个列表，如下所示。
            print("name=:",name)
            dahao = name[0]["UserName"]  #也就获取那一串的数字加字母。

            #给好友发送消息使用itchat.send()

            message_concent = 'Hey body,{} in OKex can_deposit:{},can_withdraw:{}'.format(curreny['currency'],curreny['can_deposit'],curreny['can_withdraw'])
            itchat.send(message_concent, dahao)